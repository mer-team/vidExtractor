...
function dumpYTdata(info) {
  ...
  fs.writeFile(filePath, jsonString, 'utf8', (err) => {
    if (err) {
      console.error(
        '[%s] Error writing YTDL data to JSON file: %s',
        redCrossmark,
        err,
      );
    } else {
      console.log(
        ' [%s] Information object has been written to %s.',
        greenCheckbox,
        filePath,
      );
    }
...
async function downloadAudio(videoUrl, outputFolder = './') {
  try {
    ...
    if (isValid == false) {
      console.log(
        ' [%s] The provided URL does not contain a valid YouTube ID.',
        redCrossmark,
      );
      return;
    }
    ...
    console.log(
      '  [%s] Video Length: %s secs',
      yellowInfo,
      info.videoDetails.lengthSeconds,
    );
    console.log(
      '  [%s] Video View Count: %s',
      yellowInfo,
    ...
    if (videoCategory !== 'Music') {
      console.log(
        ' [%s] The YouTube video is not a Music video (Category != Music).',
        redCrossmark,
      );
      return;
    }
    ...
    if (info.videoDetails.keywords !== null) {
      console.log(
        '  [%s] Video Keywords: %s.',
        yellowInfo,
        info.videoDetails.keywords.join(', '),
      );
    }
    ...
    if (audioFormats.length > 0) {
      ...
      const fileExtension =
        bestAudioFormat.container === 'mp4' ? 'm4a' : bestAudioFormat.container;

      // Log selected format and bitrate
      console.log(
        ` [${greenCheckbox}] Selected audio format to download: ${bestAudioFormat.container}, Bitrate: ${bestAudioFormat.audioBitrate} kbps.`,
      );
      ...
      audioStream.on('end', () => {
        console.log(
          ' [%s] Audio downloaded and saved: %s.',
          greenCheckbox,
          outputPath,
        );
      ...
      console.error(
        ' [%s] No matching audio formats found for the video.',
        redCrossmark,
      );
    }
  } catch (error) {
...
async function notifyManager(channel, queue, songId, status, payload) {
  ...
  console.log(
    ' [%s]: Result (songId: "%s" -> status: %s) sent to queue "%s"',
    greenCheckbox,
    songId,
    status,
    queue,
...
async function startService() {
  try {
    ...
    console.log(
      ' [%s] Connected to RabbitMQ server at %s:%s.',
      greenCheckbox,
      host,
      port,
    );
    ...
    console.log(
      ' [%s] Waiting for messages in %s. To exit press CTRL+C.',
      yellowInfo,
      queueName,
    );
...
if (process.argv.length == 2) {
  ...
  if (videoUrl) {
    ...
    console.error(
      ' [%s] Please provide a valid YouTube video URL as a command-line argument.',
      redCrossmark,
    );
    process.exit(1);
  }
  ...
  console.error(
    ' [%s] Please provide no arguments to start in SERVICE mode, or a YouTube video URL as a command-line argument.',
    redCrossmark,
  );
  process.exit(1);
}
